...
* [`src/ecom_front/`](src/ecom_front/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/ecom_front/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `ecom-front` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/ecom_front/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns ecom-front.example
  (:require [ecom-front.config :as config])
